...
@Component({
  selector: 'app-plate-active-work-finished',
  templateUrl: './main.html',
  styleUrls: ['./main.scss'],
})
export class Plate_active_work_Component_finished implements OnInit {
  ...
  fetchData() {
    ...
    this.apiService.get_all_work_list_new_plate_set_finished().subscribe((result: any) => {
  ...
  showModal_action(row:any): void {
    console.log('row',row)
     ...
     const dialogRef = this.dialog.open(plateset_action_Component_finished, {
       width: '80%',
       data: { Wid: this.selected_Wid }
     });
     dialogRef.afterClosed().subscribe(result => {
...
@Component({
  selector: 'app-plate-set-action-finished',
  templateUrl: './plate-action-finished.html',
  styleUrls: ['./main.scss']
})
export class plateset_action_Component_finished implements OnInit {
 ...
 // setWork: Work;
  ...
  constructor(private router: Router, private apiService: ApiService, private _httpClient: HttpClient,private formBuilder: FormBuilder, private snackBar: MatSnackBar,
              public dialogRef: MatDialogRef<plateset_action_Component_finished>, @Inject(MAT_DIALOG_DATA) public data_: any, public dialog: MatDialog) {
