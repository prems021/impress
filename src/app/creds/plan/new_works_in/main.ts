...
@Component({
  selector: 'app-dtp-active-work-in',
  templateUrl: './main.html',
  styleUrls: ['./main.scss'],
})
export class Plan_active_work_in_Component implements OnInit {
  ...
  fetchData() {
      this.apiService.get_all_plan_work_list().subscribe((result: any) => {
  ...
  showModal_action(row:any): void {
    ...
    this.selected_Wid = row.Wid;
     ...
     const dialogRef = this.dialog.open(Job_plan_action_Component, {
       width: '80%',
       data: { Wid: this.selected_Wid }
     });
     dialogRef.afterClosed().subscribe(result => {
  ...
  onRowClickEvent($event: MouseEvent, index: number,row:any): void {
    ...
    this.apiService.forward_work_from_plan(this.selected_Wid).subscribe((result: any) => {
      ...
      if (result['success'] === true) {
        ...
        this.openSnackbar(result.data, 'Ok');
      }
    });
  }
...
export class Job_plan_action_Component implements OnInit {
 ...
 // setWork: Work;
  ...
  constructor(private router: Router, private apiService: ApiService, private _httpClient: HttpClient,private formBuilder: FormBuilder, private snackBar: MatSnackBar,
              public dialogRef: MatDialogRef<Job_plan_action_Component>, @Inject(MAT_DIALOG_DATA) public data_: any, public dialog: MatDialog) {
  ...
  splitItem(item:any,k:number)
  {
   ...
   const dialogRef = this.dialog.open(Job_plan_action_split_Component, {
    width: '80%',
...
export class Job_plan_action_split_Component implements OnInit {
    ...
    this.machine_form = this.formBuilder.group(
      {
        ...
        plate_size : ['', Validators.required],
        remark : ['']
      })
